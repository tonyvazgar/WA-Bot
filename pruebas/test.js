...
const {procesarText, query} = require('./index');
...
let ejemplo;
query(mysqlConnection.connection, "PROSKIN", function(data){
    ejemplo = JSON.parse(data);
    ejemplo.forEach(element => {
        console.table(JSON.stringify(element.mensaje));
    });
});
console.log(ejemplo);